...
def build_interaction_input(video_uri, prompt):
    """Build the existing Interactions API payload in a testable helper."""
    return [
...
def main():
    ...
    ap.add_argument(
        "--check",
        action="store_true",
        help="verify credentials/model access without generating a video",
    )
    args = ap.parse_args()
    ...
    if args.check:
        try:
            ...
            sys.exit(f"Gemini connectivity check failed: {exc}")
        print(f"Gemini connectivity check passed: {getattr(model, 'name', MODEL)}")
        return
    ...
    interaction = client.interactions.create(
        model=MODEL,
        input=build_interaction_input(video_file.uri, args.prompt),
    )
